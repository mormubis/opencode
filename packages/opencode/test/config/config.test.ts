  ...
  const authWellKnown = Layer.mock(AuthWellKnown.Service)({
    ...
    configs: () =>
      Effect.gen(function* () {
        ...
        if (metadata && typeof metadata === "object" && "remote_config" in metadata && input.remote !== undefined) {
          const remote = (metadata as { remote_config: { url: string; headers?: Record<string, string> } }).remote_config
